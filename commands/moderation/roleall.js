    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permissions.has(Permissions.FLAGS.MANAGE_ROLES)) {
            const permissionEmbed = new MessageEmbed()
                .setColor('#000000')
                .setDescription('<:cross_mark:1267383549623930892> **You do not have permission to manage roles.** ');
        ...
        if (!role) {
            const mentionEmbed = new MessageEmbed()
                .setColor('#000000')
                .setDescription('<:cross_mark:1267383549623930892> **Please mention a valid role or provide a valid role ID.** ');
        ...
        const successEmbed = new MessageEmbed()
            .setColor('#000000')
            .setDescription(`<:tick:1267383653512773705> **Successfully prepared to assign role** ${role.name}.`);
        ...
        async function assignRoleToMembers(membersToAssign, role, interaction) {
            ...
            await interaction.update({ content: '<:tick:1267383653512773705> **Role assignment completed.** ', components: [] }); // Remove buttons
        }
    }
};
