...
async function untimeout(issuer, target, reason) {
    ...
    if (typeof response === 'boolean')
        return getEmbed(
            `<:tick:1267383653512773705> Successfully unmuted <@${target.user.id}>!`,
            client
        )
    if (response === 'BOT_PERM')
        return getEmbed(
            `<:cross_mark:1267383549623930892> I don't have enough permissions to unmute <@${target.user.id}>.`,
            client
        )
    else if (response === 'MEMBER_PERM')
        return getEmbed(
            `<:cross_mark:1267383549623930892> You don't have enough permissions to unmute <@${target.user.id}>.`,
            client
        )
    else if (response === 'NO_TIMEOUT')
        return getEmbed(
            `<:cross_mark:1267383549623930892> <@${target.user.id}> is not muted !`,
            client
        )
    else
        return getEmbed(
            `<:cross_mark:1267383549623930892> I don't have enough perms to unmute <@${target.user.id}>.`,
            client
        )
}
