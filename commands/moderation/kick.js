...
module.exports = {
    ...
    run: async (client, message, args) => {
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setDescription(
                            `<:warn:1267383282111086612> You must have \`Kick Members\` permissions to use this command.`
                        )
                ]
            })
        }
                ...
                return message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            .setColor(client.color)
                            .setDescription(
                                `<:warn:1267383282111086612> Please Provide Valid user ID or Mention Member.`
                            )
                    ]
                })
            }
        ...
        const teddy = new MessageEmbed()
            .setDescription(`<:cross_mark:1267383549623930892> Mention the user first`)
            .setColor(client.color)
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(client.color)
                        .setDescription(
                            `<:cross_mark:1267383549623930892> Hein? You can't kick me.`
                        )
                ]
            })
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(client.color)
                        .setDescription(
                            `<:cross_mark:1267383549623930892> I can't kick the owner of this server.`
                        )
                ]
            })
        ...
        const done = new MessageEmbed()
            .setDescription(
                 `<:tick:1267383653512773705> Successfully kicked **${user.user.tag}** from the server.`
            )
            .setColor(client.color)
