    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permissions.has('VIEW_AUDIT_LOG')) {
            const noPermissionEmbed = new MessageEmbed()
                .setColor('#000000') 
                .setDescription('<:cross_mark:1267383549623930892> **You must have `View Audit Log` permissions to use this command.**');
        ...
        if (cooldowns.has(message.author.id)) {
            const cooldownEmbed = new MessageEmbed()
                .setColor('#000000') 
                .setDescription('<:cross_mark:1267383549623930892> **Please wait before using this command again.**');
        ...
        const getEmojiForAction = (action) => {
            const emojis = {
                'MekoType': '<:text:1267739870391042068>',
                'MekoModeration': '<:hammer:1267737953774469213>',
                'MekoUser': '<:leaf:1267738126756085791>',
                'MekoTimer': '<:infinite:1267738371049132175> ',
                'MekoRuby': '<:diamond:1267738845911449601>'
            };

            return emojis[action] || '<:text:1267739870391042068>'; // Default emoji if action doesn't match
        };
            ...
            const row = new MessageActionRow()
                .addComponents(
                    new MessageButton()
                        ...
                        .setEmoji('<a:sr_loading:1260170943779835905>') // Emoji for refresh
                );
