...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const handleButtonInteraction = async (interaction) => {
            ...
            const channel = message.guild.channels.cache.get(voiceChannel.id);
            if (!channel || channel.type !== 'GUILD_VOICE') return;

            const member = message.guild.members.cache.get(interaction.user.id);
            if (!member || (!member.voice || member.voice.channelId !== channel.id)) {
                return interaction.reply({ content: 'You are not in the voice channel.', ephemeral: true });
            }
            ...
            switch (buttonId) {
                ...
                case 'userLimitButton':
                    // Ask for user limit
                    const userLimitStr = await askQuestion('Enter the maximum number of users allowed in the channel:');
                    const userLimit = parseInt(userLimitStr);
                    if (!isNaN(userLimit) && userLimit > 0) {
                        try {
                            await channel.setUserLimit(userLimit);
                            await interaction.reply({ content: `User limit set to ${userLimit} successfully.`, ephemeral: true });
                        } catch (error) {
                            console.error('Error setting user limit:', error);
                            await interaction.reply({ content: 'Failed to set user limit.', ephemeral: true });
                        }
                    } else {
                        await interaction.reply({ content: 'Invalid user limit. Please enter a valid number greater than 0.', ephemeral: true });
                    }
                    break;
                default:
                    break;
