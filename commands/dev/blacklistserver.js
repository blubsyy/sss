            ...
            return message.channel.send({
                embeds: [
                    embed
                        .setColor(client.color)
                        .setDescription(
                            `<:tick:1267383653512773705> **${user.name} (${user.id})** has been added as a **Blacklist** server.`
                        )
                ]
            })
        }
            ...
            return message.channel.send({
                embeds: [
                    embed
                        .setColor(client.color)
                        .setDescription(
                            `<:tick:1267383653512773705> **${user.name} (${user.id})** has been removed from a **Blacklist** Server.`
                        )
                ]
            })
        }
        message.channel.send({
            embeds: [
                embed
                    .setColor(client.color)
                    .setDescription(
                        `<:arrow:1267432055101460586> ${prefix}blacklistserver \`<add/remove/list>\` \`<user id>\``
                    )
            ]
        })
    }
