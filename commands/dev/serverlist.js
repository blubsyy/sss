...
const { category } = require("./eval");
const { premium } = require("./restartlogs");
...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const generateDescription = (currentGuilds) => {
      return currentGuilds
        .map((guild, index) => {
          const memberCount = guild.memberCount.toLocaleString();
          return `\`${index + 1}\` - ${guild.name} | ${memberCount} Members | ${guild.id}`;
        })
        .join("\n");
    };
    ...
    collector.on("collect", async (interaction) => {
      try {
        ...
        const firstButton = pag.components.find(
          (button) => button.customId === "first"
        );
        const previousButton = pag.components.find(
          (button) => button.customId === "previous"
        );
        const nextButton = pag.components.find(
          (button) => button.customId === "next"
        );
        const lastButton = pag.components.find(
          (button) => button.customId === "last"
        );
    ...
    collector.on("end", () => {
      ...
      msg.edit({ components: [pag] }).catch(console.error);
    });
  },
};
